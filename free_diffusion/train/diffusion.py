...
class GaussianDiffusion(nn.Module):
    def __init__(self, dtype, model, betas:np.ndarray, w:float, v:float):
        super().__init__()
        self.dtype = dtype
        self.model = model
        self.model.dtype = self.dtype
        self.betas = jt.array(betas, dtype = self.dtype)
        self.w = w
        self.v = v
        self.T = len(betas)
        ...
        self.log_tilde_betas_clipped = jt.log(jt.cat((self.tilde_betas[1].view(-1), self.tilde_betas[1:]), 0))
        ...
        self.vars = jt.cat((self.tilde_betas[1:2],self.betas[1:]), 0)
    ...
    @staticmethod
    def _extract(coef, t, x_shape:tuple):
        """
        input:
    ...
    def p_mean_variance(self, x_t, t, **model_kwargs):
        ...
        model_kwargs['cemb'] = jt.zeros(cemb_shape, device = self.device)
        pred_eps_uncond = self.model(x_t, t, **model_kwargs)
        pred_eps = (1 + self.w) * pred_eps_cond - self.w * pred_eps_uncond
        
        assert jt.isnan(x_t).int().sum() == 0, f"nan in tensor x_t when t = {t[0]}"
        assert jt.isnan(t).int().sum() == 0, f"nan in tensor t when t = {t[0]}"
        assert jt.isnan(pred_eps).int().sum() == 0, f"nan in tensor pred_eps when t = {t[0]}"
        p_mean = self._predict_xt_prev_mean_from_eps(x_t, t, pred_eps)
        p_var = self._extract(self.vars, t, x_t.shape)
        return p_mean, p_var
    ...
    def p_sample(self, x_t, t, **model_kwargs):
        ...
        assert jt.isnan(mean).int().sum() == 0, f"nan in tensor mean when t = {t[0]}"
        assert jt.isnan(var).int().sum() == 0, f"nan in tensor var when t = {t[0]}"
    ...
    def sample(self, shape:tuple, **model_kwargs):
        ...
        if model_kwargs == None:
            model_kwargs = {}
        x_t = jt.randn(shape, device = self.device)
        tlist = jt.ones([x_t.shape[0]], device = self.device) * self.T
    ...
    def ddim_p_mean_variance(self, x_t, t, prevt, eta:float, **model_kwargs):
        """
        calculate the parameters of p_{theta}(x_{t-1}|x_t)
        """
        if model_kwargs == None:
    ...
    def ddim_p_sample(self, x_t, t, prevt, eta:float, **model_kwargs): 
        if model_kwargs == None:
            model_kwargs = {}
        B, C = x_t.shape[:2]
        assert t.shape == (B,), f"size of t is not batch size {B}"
        mean, var = self.ddim_p_mean_variance(x_t , t, prevt, eta, **model_kwargs)
        assert jt.isnan(mean).int().sum() == 0, f"nan in tensor mean when t = {t[0]}"
        assert jt.isnan(var).int().sum() == 0, f"nan in tensor var when t = {t[0]}"
        noise = jt.randn_like(x_t)
        noise[t <= 0] = 0 
    ...
    def ddim_sample(self, shape:tuple, num_steps:int, eta:float, select:str, **model_kwargs):
        ...
        x_t = jt.randn(shape, device = self.device)
        tlist = jt.zeros([x_t.shape[0]], device = self.device)
        for i in tqdm(range(num_steps),dynamic_ncols=True):
            with jt.no_grad():
                tlist = tlist * 0 + tseq[-1-i]
                if i != num_steps - 1:
                    prevt = jt.ones_like(tlist, device = self.device) * tseq[-2-i]
                else:
                    prevt = - jt.ones_like(tlist, device = self.device) 
