...
class AttentionPool2d(jt.Module):
...
class ResBlock(TimestepBlock):
    ...
    #     """
    #     return checkpoint(
    #         self._execute, (x, emb), self.parameters(), self.use_checkpoint
    #     )

    def execute(self, x, emb):
        if self.updown:
            in_rest, in_conv = nn.Sequential(list(self.in_layers.layers.values())[:-1]), list(self.in_layers.layers.values())[-1]
        ...
        if self.use_scale_shift_norm:
            #print(list(self.out_layers.layers.values())[1:])
            out_norm, out_rest = self.out_layers.layers['0'], nn.Sequential(list(self.out_layers.layers.values())[1:])
...
class AttentionBlock(jt.Module):
    ...
    def execute(self, x):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)
        norm = self.norm(x).reshape(b, c, -1)
        qkv = self.qkv(norm)
...
def count_flops_attn(model, _x, y):
    ...
    model.total_ops += jt.array([matmul_ops])



class QKVAttentionLegacy(jt.Module):
...
class EncoderUNetModel(jt.Module):
